...
export class ClassificationModule extends LitModule {
   ...
   */
  private parseResult(model: string, inputs: IndexedInput[]): LabeledPredictions {
    ...
    for (const predKey of multiclassKeys) {
      ...
      const scores = inputs.map(input => this.dataService.getVal(
          input.id, topLevelKey));
      const predictedClasses = inputs.map(input => this.dataService.getVal(
          input.id, predClassKey));
      const labels = vocab ||
          Array.from({length: scores[0].length}, (v, k) => `${k}`);
